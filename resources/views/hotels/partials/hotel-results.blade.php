...
@if($hasTravelAgentUser)
    {{-- ========================================================
         AGENT VIEW: Executive Data-Table List
    ======================================================== --}}
    <div class="rounded-2xl bg-white shadow-sm border border-slate-200 overflow-hidden">
        <div class="p-4 border-b border-slate-100 bg-slate-50/70 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
                <span class="text-xs font-bold uppercase tracking-wider text-slate-700">
                    Available Hotels ({{ isset($hotels) ? count($hotels) : 0 }})
                </span>
                <p class="text-xs text-slate-400 font-medium">Real-time hotel inventory and contracted agent rates</p>
            </div>
            <div class="flex items-center gap-2">
                <span class="text-xs font-semibold text-slate-500">Destination:</span>
        ...
        <div class="overflow-x-auto">
            @if(!isset($hotels) || count($hotels) === 0)
                <div class="p-12 text-center text-slate-400">
                    <div class="inline-flex items-center justify-center w-14 h-14 rounded-full bg-slate-100 text-slate-400 mb-3 text-2xl">
                        🏨
                    </div>
                    <h3 class="text-base font-bold text-slate-800">No Hotels Found</h3>
                    <p class="mt-1 text-sm text-slate-500">No properties matched your destination and date criteria. Try adjusting the search filters.</p>
                </div>
            @else
                <table class="min-w-full divide-y divide-slate-200 text-sm">
                    <thead class="bg-slate-50 text-slate-600 text-[11px] uppercase tracking-wider font-bold">
                        <tr>
                            <th class="px-5 py-3.5 text-left">Hotel Property</th>
                            <th class="px-5 py-3.5 text-left">City</th>
                            <th class="px-5 py-3.5 text-left">Category</th>
                            <th class="px-5 py-3.5 text-left">Distance from Haram</th>
                            <th class="px-5 py-3.5 text-left">Inclusions</th>
                            <th class="px-5 py-3.5 text-left">Starting Rate</th>
                    ...
                    <tbody class="divide-y divide-slate-100 bg-white">
                        @foreach($hotels as $hotel)
                            @php
                                $price = optional($hotel->bestRoomType)->daily_rate ?? ($hotel->roomTypes?->min('daily_rate') ?? 0);
                                $starText = match($hotel->category) {
                                    '5 Star' => '★★★★★ (5 Star)',
                                    '4 Star' => '★★★★☆ (4 Star)',
                                    '3 Star' => '★★★☆☆ (3 Star)',
                                    default => $hotel->category ?: 'Standard'
                                };
                            @endphp
                            <tr class="hover:bg-blue-50/40 transition duration-150">
                                ...
                                <td class="px-5 py-4">
                                    <div class="flex items-center gap-3">
                                        <div class="h-11 w-11 rounded-xl bg-slate-100 overflow-hidden flex-shrink-0 border border-slate-200">
                                            @if($hotel->cover_image_url)
                                                <img src="{{ $hotel->cover_image_url }}" alt="{{ $hotel->hotel_name }}" class="h-full w-full object-cover">
                                            @else
                                                <div class="h-full w-full flex items-center justify-center text-slate-400 font-bold text-base">🏨</div>
                                            @endif
                                        </div>
                                        <div>
                                            <span class="block font-bold text-slate-900 text-sm">{{ $hotel->hotel_name }}</span>
                                            <div class="flex items-center gap-1.5 mt-0.5">
                                                <span class="text-[10px] text-slate-400 font-mono">Code: {{ $hotel->hotel_code ?: '#'.$hotel->id }}</span>
                                                @if($hotel->featured)
                                                    <span class="inline-flex items-center px-1.5 py-0.2 rounded text-[9px] font-bold bg-amber-50 text-amber-700 border border-amber-200">Featured</span>
                                                @endif
                                            </div>
                                        </div>
                                    </div>
                                ...
                                {{-- City --}}
                                <td class="px-5 py-4">
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold {{ $hotel->city === 'Makkah' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-indigo-50 text-indigo-700 border border-indigo-200' }}">
                                        {{ $hotel->city ?: 'KSA' }}
                                    </span>
                                </td>
                                ...
                                {{-- Category --}}
                                <td class="px-5 py-4">
                                    <span class="text-xs font-bold text-amber-500 block">{{ $starText }}</span>
                                </td>

                                {{-- Distance from Haram --}}
                                <td class="px-5 py-4">
                                    <div class="text-xs font-semibold text-slate-800 flex items-center gap-1">
                                        <svg class="w-3.5 h-3.5 text-blue-600" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clip-rule="evenodd"></path></svg>
                                        {{ (float)$hotel->distance_from_haram }} KM
                                    </div>
                                    <span class="text-[10px] text-slate-400">from Haram</span>
                                </td>
                                    ...
                                    <div class="flex flex-wrap gap-1 max-w-[200px]">
                                        ...
                                        @if($hotel->mealPlans?->isNotEmpty())
                                            <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-semibold bg-blue-50 text-blue-700 border border-blue-200">
                                                {{ $hotel->mealPlans->first()->meal_plan_name }}
                                            </span>
                                        @else
                                ...
                                <td class="px-5 py-4">
                                    ...
                                    <div class="text-[10px] text-slate-400 font-medium">/ night</div>
                                </td>
                                ...
                                <td class="px-5 py-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        <a
                                            href="{{ route('hotels.booking.create', $hotel) }}"
                                        ...
                                        <a
                                            href="{{ route('hotels.details', $hotel) }}"
                                            class="inline-flex items-center justify-center rounded-xl border border-slate-200 bg-white px-2.5 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50 transition shadow-xs"
...
@else
    {{-- ========================================================
         CUSTOMER VIEW: Shopping Cards View
    ======================================================== --}}
    <div class="space-y-6">
        <!-- Top Filter/Sort Bar -->
