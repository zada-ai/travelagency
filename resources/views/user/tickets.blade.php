@php
    ...
    $portalSystemLabel = 'Customer Portal System';
@endphp
...
@section('content')
    <div class="mx-auto max-w-[1450px] px-5 py-6 sm:px-7 lg:px-9">
        ...
        <section class="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm sm:p-6 mb-6">
            ...
            <form method="GET" action="{{ route('tickets.index') }}" class="grid gap-4 lg:grid-cols-12">
        ...
        @if($tickets->isEmpty())
            <section class="rounded-[24px] border border-slate-200 bg-white p-12 text-center shadow-sm">
            ...
            <div class="space-y-4">
                @foreach($tickets as $ticket)
                    @php
                        ...
                        $routeLabel = $ticket->route ?: trim(($ticket->departureAirport?->city ?? '') . ' - ' . ($ticket->arrivalAirport?->city ?? ''));
                        $departureCode = $ticket->departureAirport?->code ?? '---';
                        $arrivalCode = $ticket->arrivalAirport?->code ?? '---';
                    @endphp

                    <article class="overflow-hidden rounded-[26px] border border-slate-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg">
                                    ...
                                    <div class="mt-4 flex flex-wrap items-center gap-3 rounded-2xl border border-blue-100 bg-blue-50/60 px-4 py-3">
                                        ...
                                        @if($ticket->return_departure_time)
                                            ...
                                            <span class="text-sm text-slate-600">
                                                {{ $ticket->return_departure_time }} → {{ $ticket->return_arrival_time }}
                                            </span>
                                        @endif
                                    </div>
                                @endif
                                    ...
                                    <div class="mt-5 space-y-2">
                                        @if($economyPrice !== null)
                                            <div class="flex items-center justify-between rounded-xl bg-white px-3 py-2 text-xs border border-slate-100">
                                                ...
                                                <span class="font-bold text-slate-900">SAR {{ number_format($economyPrice, 0) }}</span>
                                            </div>
                                        @endif
                                        ...
                                        @if($premiumPrice !== null)
                                            <div class="flex items-center justify-between rounded-xl bg-white px-3 py-2 text-xs border border-slate-100">
                                                ...
                                                <span class="font-bold text-slate-900">SAR {{ number_format($premiumPrice, 0) }}</span>
                                            </div>
                                        @endif
                                        ...
                                        @if($businessPrice !== null)
                                            <div class="flex items-center justify-between rounded-xl bg-white px-3 py-2 text-xs border border-slate-100">
                                                ...
                                                <span class="font-bold text-slate-900">SAR {{ number_format($businessPrice, 0) }}</span>
                                            </div>
                                        @endif
                                        ...
                                        @if($firstPrice !== null)
                                            <div class="flex items-center justify-between rounded-xl bg-white px-3 py-2 text-xs border border-slate-100">
                                                ...
                                                <span class="font-bold text-slate-900">SAR {{ number_format($firstPrice, 0) }}</span>
                                            </div>
                                        @endif
                                    </div>
