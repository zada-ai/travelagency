...
<body>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-4 gap-4">
                        ...
                        <div class="relative">
                            ...
                            <div class="relative">
                                ...
                                <select class="form-select w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 text-sm font-medium h-[50px]" id="destinationSelect">
                                    <option value="Makkah" selected>Makkah Al Mukarramah</option>
                        ...
                        <div class="grid grid-cols-2 gap-2 lg:col-span-2">
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="text" class="w-full pl-10 pr-3 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium h-[50px]" id="checkIn" placeholder="YYYY-MM-DD" readonly>
                                </div>
                            </div>
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="text" class="w-full pl-10 pr-3 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium h-[50px]" id="checkOut" placeholder="YYYY-MM-DD" readonly>
                                </div>
                            </div>
                        </div>
                ...
                <div class="bg-white rounded-2xl border border-slate-200 p-4 space-y-6 shadow-sm sticky top-6">
                    ...
                    <div>
                        ...
                        <div class="space-y-2">
                            ...
                            <label class="flex items-center gap-2 text-sm text-slate-600 cursor-pointer hover:text-slate-900">
                                <input type="checkbox" class="rounded text-blue-600 focus:ring-blue-500 border-slate-300 w-4 h-4">
                                <span class="text-yellow-500 font-serif">★★☆☆☆</span> <span class="text-xs text-slate-400">(2 Star)</span>
                            </label>
                            <label class="flex items-center gap-2 text-sm text-slate-600 cursor-pointer hover:text-slate-900">
                                <input type="checkbox" class="rounded text-blue-600 focus:ring-blue-500 border-slate-300 w-4 h-4">
                                <span class="text-yellow-500 font-serif">★☆☆☆☆</span> <span class="text-xs text-slate-400">(1 Star)</span>
                            </label>
                        </div>
                    </div>
                    ...
                    <!-- Refactored Clean Budget Price Slider Component -->
                    <div>
                        <h6 class="text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">Price Range (SAR)</h6>
                        <input type="range" class="form-range accent-blue-600 w-full bg-slate-100 rounded-lg appearance-none cursor-pointer h-2" min="0" max="2000" step="50" id="priceRange" value="1200">
                        <div class="flex justify-between items-center text-xs text-slate-500 mt-2 font-semibold">
                            <span>0 SAR</span>
                            <span class="text-blue-600 bg-blue-50 px-2 py-0.5 rounded shadow-sm border border-blue-100" id="rangeValDisplay">Max: 1200 SAR</span>
                            <span>2000 SAR</span>
                        </div>
                    </div>
                                ...
                                <div>
                                    <div class="flex flex-wrap items-center justify-between gap-2 mb-2">
                                        ...
                                        <div class="flex items-center gap-1.5 bg-blue-50 px-2.5 py-1 rounded-lg">
                                            <span class="text-sm font-bold text-blue-700">{{ number_format($hotel->featured ? 4.8 : 4.3, 1) }}</span>
                                            <span class="text-xs font-semibold text-blue-600">{{ $hotel->featured ? 'Excellent' : 'Very Good' }}</span>
                                        </div>
                                    </div>

                                    <p class="text-sm text-slate-500 font-medium mb-3 flex items-center gap-1.5">
                                        <i class="bi bi-cursor-fill text-blue-600"></i> Distance from Haram: <strong>{{ number_format($hotel->distance_from_haram ?? 0, 0) }} meters</strong>
                                    </p>
                                    ...
                                    <div class="flex flex-wrap items-center gap-2 mb-4">
                                        ...
                                        <span class="bg-slate-100 text-slate-700 text-xs font-semibold px-2.5 py-1 rounded-full"><i class="bi bi-wifi"></i> WiFi</span>
                                        <span class="bg-slate-100 text-slate-700 text-xs font-semibold px-2.5 py-1 rounded-full"><i class="bi bi-snow"></i> AC</span>
                                    </div>
                                ...
                                <div class="flex flex-col sm:flex-row sm:items-center justify-between border-t border-slate-100 pt-4 mt-4 gap-4">
                                    <div>
                                        ...
                                        <div class="flex items-baseline gap-1">
                                            <span class="text-2xl font-extrabold text-slate-900">SAR 450.00</span>
                                            <span class="text-xs text-slate-400 font-semibold">Excl. VAT</span>
                                        </div>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <button class="bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold px-4 py-2.5 rounded-xl text-sm transition">View Details</button>
                                        <button class="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-5 py-2.5 rounded-xl text-sm shadow-md shadow-blue-600/10 transition">Book Now</button>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <!-- INTERACTIVE DOM FUNCTIONALITY SCRIPTS -->
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Price Range Display Helper configuration
            const priceSlider = document.getElementById('priceRange');
            const rangeValDisplay = document.getElementById('rangeValDisplay');
            if(priceSlider && rangeValDisplay) {
                rangeValDisplay.textContent = `Max: ${priceSlider.value} SAR`;
                priceSlider.addEventListener('input', (e) => {
                    rangeValDisplay.textContent = `Max: ${e.target.value} SAR`;
                });
            }

            // Flatpickr initialization matrix
            ...
            function synchronizeDOMChanges() {
                ...
                // Adult Dynamic Name Field Matrix Injection
                const adultBox = document.getElementById('adultInputsContainer');
                adultBox.innerHTML = '';
                if (a > 1) {
                    const header = document.createElement('p');
                    header.className = 'text-xs font-bold uppercase text-blue-600 tracking-wider mb-2 mt-2';
                    header.textContent = 'Additional Guest Names';
                    adultBox.appendChild(header);
                    
                    for (let step = 2; step <= a; step++) {
                        const div = document.createElement('div');
                        div.className = 'flex flex-col gap-1 mb-2 bg-slate-50 p-2 rounded-lg border border-slate-100';
                        div.innerHTML = `
                            <span class="text-xs font-semibold text-slate-700">Adult ${step}:</span>
                ...
                // Infant Dynamic Input Generation Structure
                if (i > 0) {
                    const infantHeader = document.createElement('p');
                    infantHeader.className = 'text-xs font-bold uppercase text-blue-600 tracking-wider mb-2 mt-3';
                    infantHeader.textContent = 'Infant Details';
                    adultBox.appendChild(infantHeader);

                    for (let step = 1; step <= i; step++) {
                        const div = document.createElement('div');
                        div.className = 'flex flex-col gap-2 mb-2 bg-blue-50/50 p-2 rounded-lg border border-blue-100';
                        div.innerHTML = `
                            <span class="text-xs font-semibold text-slate-700">Infant ${step}:</span>
                            <input type="text" class="form-control form-control-sm border-slate-200 rounded-lg text-xs" placeholder="Infant Name">
                            <input type="text" class="form-control form-control-sm border-slate-200 rounded-lg text-xs" placeholder="Passport Number (Optional)">
                        `;
                        adultBox.appendChild(div);
                    }
                }

                // Child Dynamic Age Selection Selectors Engine
                ...
                if (c > 0) {
                    ...
                    for (let step = 1; step <= c; step++) {
                        ...
                        for(let age = 0; age <= 17; age++) { 
                            selectOpts += `<option value="${age}">${age} Years Old</option>`; 
                        }
                        div.innerHTML = `
                            <span class="text-xs text-slate-600 font-medium">Child ${step} Age:</span>
            ...
            // Execute synchronous baseline setup
            synchronizeDOMChanges();
        });
    </script>
</body>
