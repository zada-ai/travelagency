...
<head>
    ...
    <title>Create Visa Application | Agent Portal</title>
            ...
            <main class="p-4 sm:p-6 lg:p-8 space-y-6 overflow-x-hidden">
                <div class="max-w-6xl mx-auto space-y-6">
                    <nav class="flex items-center gap-2 text-sm text-slate-500">
                        ...
                        <span>→</span>
                        <span class="text-blue-600 font-semibold">Create Visa Application</span>
                    </nav>
                    ...
                    <section class="glass-panel rounded-3xl p-6 md:p-8 shadow-xs">
                        <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                            <div>
                                ...
                                <h1 class="mt-2 text-3xl font-extrabold text-slate-900">Create New Visa Application</h1>
                                <p class="mt-2 text-sm text-slate-500">Submit a visa application for your agency customer and upload required documents.</p>
                            </div>
                        </div>
                        ...
                        <form action="{{ route('travel-agents.visa-applications.store') }}" method="POST" enctype="multipart/form-data" class="mt-8 space-y-6">
                            @csrf

                            <div class="grid gap-6 lg:grid-cols-2">
                                <label class="block">
                                    <span class="text-sm font-semibold text-slate-700">Customer Name</span>
                                    <input name="customer_name" value="{{ old('customer_name') }}" type="text" placeholder="John Doe" class="mt-2 w-full rounded-3xl border border-slate-200 bg-white px-4 py-3 text-slate-800 shadow-sm focus:border-blue-500 focus:outline-none" required>
                                </label>

                                <label class="block">
                                    <span class="text-sm font-semibold text-slate-700">Passport Number</span>
                                    <input name="passport_number" value="{{ old('passport_number') }}" type="text" placeholder="A1234567" class="mt-2 w-full rounded-3xl border border-slate-200 bg-white px-4 py-3 text-slate-800 shadow-sm focus:border-blue-500 focus:outline-none" required>
                                </label>

                                <label class="block">
                                    <span class="text-sm font-semibold text-slate-700">Passport Expiry</span>
                                    <input name="passport_expiry" value="{{ old('passport_expiry') }}" type="date" class="mt-2 w-full rounded-3xl border border-slate-200 bg-white px-4 py-3 text-slate-800 shadow-sm focus:border-blue-500 focus:outline-none" required>
                                </label>

                                <label class="block">
                                    <span class="text-sm font-semibold text-slate-700">Nationality</span>
                                    <input name="nationality" value="{{ old('nationality') }}" type="text" placeholder="Pakistan" class="mt-2 w-full rounded-3xl border border-slate-200 bg-white px-4 py-3 text-slate-800 shadow-sm focus:border-blue-500 focus:outline-none" required>
                                </label>
                                ...
                                <label class="block lg:col-span-2">
                                    ...
                                    <select name="visa_type_id" required class="mt-2 w-full rounded-3xl border border-slate-200 bg-white px-4 py-3 text-slate-800 shadow-sm focus:border-blue-500 focus:outline-none">
                                        ...
                                        @foreach($visaTypes as $visaType)
                                            <option value="{{ $visaType->id }}" {{ old('visa_type_id') == $visaType->id ? 'selected' : '' }}>{{ $visaType->name }} - SAR {{ number_format($visaType->base_fee + $visaType->service_charge, 2) }}</option>
                                        @endforeach
                                    </select>
                                </label>
                            </div>
                            ...
                            <div class="grid gap-6 md:grid-cols-2">
                                <label class="block">
                                    ...
                                    <input type="file" name="passport_copy" class="mt-2 w-full rounded-3xl border border-slate-200 bg-white px-4 py-3 text-slate-800 shadow-sm focus:border-blue-500 focus:outline-none" accept="image/jpeg,image/png,application/pdf" required>
                                </label>

                                <label class="block">
                                    <span class="text-sm font-semibold text-slate-700">Photograph</span>
                                    <input type="file" name="photograph" class="mt-2 w-full rounded-3xl border border-slate-200 bg-white px-4 py-3 text-slate-800 shadow-sm focus:border-blue-500 focus:outline-none" accept="image/jpeg,image/png" required>
                                </label>
                            ...
                            <div>
                                <label class="block">
                                    ...
                                    <textarea name="remarks" rows="4" class="mt-2 w-full rounded-3xl border border-slate-200 bg-white px-4 py-3 text-slate-800 shadow-sm focus:border-blue-500 focus:outline-none" placeholder="Enter any notes for the admin or customer">{{ old('remarks') }}</textarea>
                                </label>
                            </div>

                            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                <a href="{{ route('travel-agents.visa-applications') }}" class="inline-flex items-center justify-center rounded-3xl border border-slate-200 bg-white px-6 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50 transition">Back to applications</a>
                                <button type="submit" class="inline-flex items-center justify-center rounded-3xl bg-blue-600 px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-blue-500/20 hover:bg-blue-700 transition">Submit Application</button>
                            </div>
                        </form>
                    </section>
                </div>
