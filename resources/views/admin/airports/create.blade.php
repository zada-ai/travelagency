...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        <div class="mb-6">
            <h1 class="text-2xl font-semibold text-slate-900">Add Airport</h1>
            <p class="mt-2 text-sm text-slate-500">Create a new airport master record for flight inventory.</p>
        </div>

        <form action="{{ route('admin.airports.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                <label class="block">
                    ...
                    <input type="text" name="city" value="{{ old('city') }}" class="mt-2 w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-slate-900 focus:border-blue-500 focus:outline-none" />
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                ...
                <label class="block">
                    ...
                    <input type="text" name="timezone" value="{{ old('timezone') }}" class="mt-2 w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-slate-900 focus:border-blue-500 focus:outline-none" />
            ...
            <div class="flex items-center justify-between gap-3 pt-4">
                ...
                <button type="submit" class="inline-flex items-center justify-center rounded-xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white transition hover:bg-slate-800">Save Airport</button>
            </div>
        </form>
    </div>
</div>
