...
@section('title', 'New Seasonal Rate')

@section('content')
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-semibold text-slate-900">Add Seasonal Rate</h1>
            <p class="text-sm text-slate-500">Create a new seasonal price for a hotel room type.</p>
        </div>
    ...
    <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        <form method="POST" action="{{ route('admin.hotel-seasonal-rates.store') }}" class="grid gap-6">
            @csrf
                ...
                <div>
                    ...
                    <select name="hotel_room_type_id" class="mt-1 block w-full rounded-md border-slate-300 bg-white px-3 py-2 text-sm shadow-sm focus:border-slate-500 focus:outline-none focus:ring-1 focus:ring-slate-500">
                        ...
                        @foreach($roomTypes as $roomType)
                            <option value="{{ $roomType->id }}" {{ old('hotel_room_type_id') == $roomType->id ? 'selected' : '' }}>{{ $roomType->room_name }}</option>
                        @endforeach
                    </select>
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <input type="text" name="season_name" value="{{ old('season_name') }}" class="mt-1 block w-full rounded-md border-slate-300 px-3 py-2 text-sm shadow-sm focus:border-slate-500 focus:outline-none focus:ring-1 focus:ring-slate-500" placeholder="Peak Season" />
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <input type="date" name="start_date" value="{{ old('start_date') }}" class="mt-1 block w-full rounded-md border-slate-300 px-3 py-2 text-sm shadow-sm focus:border-slate-500 focus:outline-none focus:ring-1 focus:ring-slate-500" />
                ...
                <div>
                    ...
                    <input type="date" name="end_date" value="{{ old('end_date') }}" class="mt-1 block w-full rounded-md border-slate-300 px-3 py-2 text-sm shadow-sm focus:border-slate-500 focus:outline-none focus:ring-1 focus:ring-slate-500" />
            ...
            <div class="flex justify-end gap-3 pt-2">
                ...
                <button type="submit" class="inline-flex items-center justify-center rounded-md bg-slate-900 px-4 py-2 text-sm font-medium text-white transition hover:bg-slate-700">Save seasonal rate</button>
            </div>
        </form>
    </div>
</div>
