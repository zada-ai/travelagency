@php
    ...
    $portalLabel = $hasTravelAgentUser ? 'Agent Portal' : ($isCustomer ? 'Customer Portal' : 'Admin Portal');
    $portalSystemLabel = $hasTravelAgentUser ? 'Agent Portal System' : ($isCustomer ? 'Customer Portal System' : 'Admin Portal System');
    $backRoute = request()->routeIs('travel-agents.packages.book') ? route('travel-agents.packages.index') : route('packages.index');
@endphp
...
@section('content')
    <div class="space-y-6">
        <div class="bg-white rounded-3xl p-6 md:p-8 shadow-xs border border-slate-200/80 flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <a href="{{ $backRoute }}" class="px-5 py-2 border border-gray-300 text-gray-600 rounded-full hover:bg-gray-50 transition text-sm font-medium">
                Cancel
            </a>
        </div>
        ...
        <form action="{{ $bookingStoreRoute }}" method="POST" enctype="multipart/form-data" id="bookingForm">
            ...
            @if($package->hotelStays->count())
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden mb-6">
                    <div class="border-b border-gray-100 px-6 py-4 bg-gray-50/50">
                        <h5 class="text-base font-bold text-gray-800">4. Hotel Accommodation</h5>
                        <p class="text-xs text-gray-500 mt-0.5">Hotels included in this package</p>
                    </div>
                            ...
                            <div class="border border-gray-200 rounded-xl p-5 bg-slate-50">
                                <div class="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                                    <div>
                                        <div class="flex items-center gap-2">
                                            <h6 class="font-bold text-gray-900 text-base">{{ $stay->hotel_name }}</h6>
                                            @if($stay->star_rating)
                                                <span class="text-amber-500 text-sm">{{ str_repeat('★', (int) $stay->star_rating) }}</span>
                                            @endif
                                        </div>
                                        <p class="text-sm text-gray-500 mt-1"><i class="bi bi-geo-alt"></i> {{ $stay->city }}</p>
                                    </div>

                                    @if($stay->price_per_person)
                                        <div class="text-right">
                                            <span class="text-xs text-gray-400 block">Hotel Price / Person</span>
                                            <span class="font-bold text-indigo-600">SAR {{ number_format($stay->price_per_person, 2) }}</span>
                                        </div>
                                    @endif
                                </div>
                                ...
                                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mt-5 pt-4 border-t border-gray-200">
                                    <div>
                                        ...
                                        <span class="text-sm font-semibold text-gray-800">{{ $stay->check_in?->format('d M Y') ?? 'N/A' }}</span>
                                    </div>
                                    <div>
                                        <span class="text-xs text-gray-400 block">Check-out</span>
                                        <span class="text-sm font-semibold text-gray-800">{{ $stay->check_out?->format('d M Y') ?? 'N/A' }}</span>
                                    </div>
                                    <div>
                                        <span class="text-xs text-gray-400 block">Nights</span>
                                        <span class="text-sm font-semibold text-gray-800">{{ $stay->nights ?? 'N/A' }}</span>
                                    </div>
                                    <div>
                                        <span class="text-xs text-gray-400 block">Room Type</span>
                                        <span class="text-sm font-semibold text-gray-800">{{ $stay->room_type ?? 'N/A' }}</span>
                                    </div>
                                </div>
                                ...
                                @if($stay->distance_from_haram || $stay->walking_time)
                                    <div class="flex flex-wrap gap-4 mt-4 text-xs text-gray-500">
                                        @if($stay->distance_from_haram)
                                            <span><i class="bi bi-signpost-2"></i> {{ $stay->distance_from_haram }} from Haram</span>
                                        @endif
                                        @if($stay->walking_time)
                                            <span><i class="bi bi-person-walking"></i> {{ $stay->walking_time }} walking</span>
                                        @endif
                                    </div>
                                @endif
                            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden mb-6">
                <div class="border-b border-gray-100 px-6 py-4 bg-gray-50/50">
                    <h5 class="text-base font-bold text-gray-800">5. Select Seats</h5>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden mb-6">
                <div class="border-b border-gray-100 px-6 py-4 bg-gray-50/50 flex items-center justify-between">
                    <div>
                        <h5 class="text-base font-bold text-gray-800">6. Passenger Details</h5>
